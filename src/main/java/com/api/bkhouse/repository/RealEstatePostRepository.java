...
public interface RealEstatePostRepository extends JpaRepository<RealEstatePost, UUID> {
    ...
    @Modifying
    @Query(value = "update real_estate_posts set is_enabled = false, status = 'DA_HET_HAN' \n" +
            "where now() > cast(metadata->>'expired_at' as timestamptz) \n" +
            "and is_enabled = true\n" +
            "and status IN ('CHO_KIEM_DUYET', 'DA_KIEM_DUYET', 'BI_TU_CHOI')", nativeQuery = true)
    ...
    Optional<RealEstatePost> findByIdAndEnabled(UUID id, Boolean enabled);
    ...
    @Query(value = "select CAST(rep.id AS varchar) as id, rep.type, rep.is_sell as sell, rep.status, rep.is_enabled as enable, rep.price, rep.area, rep.created_at as createAt, " +
            ...
            "(select media_url from post_media where post_id = rep.id limit 1) as imageUrl, " +
    ...
    @Query(value = "select rep.id, rep.title, rep.price, rep.area, rep.is_sell as sell, rep.address_show as addressShow, rep.created_at as createAt, " +
    ...
    @Query(value = "select rep.id, rep.is_sell as sell, rep.type, rep.status, rep.is_enabled as enable, rep.price, rep.area, rep.created_at as createAt,\n" +
            "concat(u.first_name, ' ', u.middle_name, ' ', u.last_name) as fullName,\n" +
            "u.phone_number as phoneNumber\n" +
            "from real_estate_posts rep \n" +
            "inner join real_estate_post_price repp on rep.id = repp.real_estate_post_id\n" +
            "inner join users u on rep.owner_id = u.id\n" +
            "group by rep.id, rep.type, rep.status, rep.is_enabled, rep.price, rep.area, rep.created_at, u.first_name, u.middle_name, u.last_name, u.phone_number\n" +
    ...
    @Query(value = "select rep.id, rep.title, rep.is_sell as sell, rep.price, rep.description, rep.is_enabled as enable, rep.type, \n" +
    ...
    @Query(value = "select u.id, u.phone_number as phoneNumber, u.email, u.avatar_url as avatarUrl, i.create_at as createAt, \n" +
            ...
            "where i.user_id::text != 'anonymous'\n" +
