...
public interface PostPayRepository extends JpaRepository<PostPay, Long> {
    ...
    @Query(value = "select tmp1.postPrice, tmp2.specialAccountPay from\n" +
            "(select sum(price) as postPrice from post_pay where month(create_at) = :month and extract(year from create_at) = :year) as tmp1,\n" +
            "(select sum(amount) as specialAccountPay from special_account_pay where month(create_at) = :month and extract(year from create_at) = :year) as tmp2;", nativeQuery = true)
    ...
    @Query(value = "select sum(so_tien) from charge where status = 'DA_XAC_NHAN' and month(create_at) = :month and extract(year from create_at) = :year", nativeQuery = true)
